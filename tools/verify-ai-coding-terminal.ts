...
assert.match(handler, /function isTrustedAiCodingIpcSender\(event: IpcMainInvokeEvent\): boolean/)
assert.match(handler, /ownerWindow\.webContents === event\.sender/)
assert.match(handler, /event\.senderFrame === event\.sender\.mainFrame/)
assert.match(handler, /ai-coding:terminal-launch'[\s\S]*isTrustedAiCodingIpcSender\(event\)/)
assert.match(handler, /Unauthorized AI coding IPC sender/)
