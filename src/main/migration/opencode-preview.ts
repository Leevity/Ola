...
function readProviderPersistState(): ProviderPersistState {
  ...
  const bucket = isPlainObject(config['ola-providers'])
    ? (config['ola-providers'] as Record<string, unknown>)
    : {}
...
function createProviderDraft(source: OpenCodeSourceProvider): OlaProviderDraft {
  const builtinId = inferBuiltinId(source)
  ...
  const models = source.models.map((model) => toOlaModel(source, model))
