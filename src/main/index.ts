...
import { registerMediaRuntimeHandlers } from './ipc/media-runtime-handlers'
...
function resolveRendererHeapLimitMb(systemMemMb: number): number {
  ...
  const envOverride = parsePositiveInteger(process.env.OLA_RENDERER_HEAP_MB)
  if (envOverride) {
...
function configureRendererHeapLimit(): void {
  try {
    ...
    if (process.env.OLA_DISABLE_MEMORY_PRESSURE === '1') {
...
if (gotSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    ...
    runLoggedStartupStep('set_app_user_model_id', () => {
      electronApp.setAppUserModelId('com.ola.app')
    })
