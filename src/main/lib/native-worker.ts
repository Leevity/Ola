...
class NativeWorkerManager {
  ...
  private async start(): Promise<void> {
    ...
    if (!workerPath) {
      throw new Error('Native worker is missing. Run `npm run native:publish` before starting Ola.')
    }
    ...
    try {
      ...
      throw new Error(
        [
          ...
          'The running binary is likely stale; run `npm run native:publish` and restart Ola.',
    ...
    if (missing.length > 0) {
      throw new Error(
        [
          ...
          'Run `npm run native:publish` and restart Ola.'
        ].join(' ')
      )
    }
...
function createNativeWorkerEnv(): NodeJS.ProcessEnv {
  ...
  if (readBooleanEnv('OLA_NATIVE_DEBUG') === null && !app.isPackaged) {
    env.OLA_NATIVE_DEBUG = '1'
  }
  env.OLA_APP_VERSION = app.getVersion().trim()
  env.OLA_NATIVE_SLOW_MS ??= String(getNativeWorkerSlowRequestMs())
  return env
}
...
function isNativeWorkerDebugEnabled(): boolean {
  return readBooleanEnv('OLA_NATIVE_DEBUG') ?? !app.isPackaged
}

function isMessagePackTraceEnabled(): boolean {
  return readBooleanEnv('OLA_MSGPACK_TRACE') ?? false
}

function getNativeWorkerSlowRequestMs(): number {
  const raw = process.env.OLA_NATIVE_SLOW_MS
...
function resolveNativeWorkerPath(): string | null {
  const overridePath = process.env.OLA_NATIVE_WORKER_PATH?.trim()
  if (overridePath && fs.existsSync(overridePath)) {
    return overridePath
  }

  const executableName =
    process.platform === 'win32' ? 'Ola.Native.Worker.exe' : 'Ola.Native.Worker'
