...
function voiceParamsFromConfig(): PetVoiceParams | null {
  // Prefer the active pet's voice config (new model); fall back to the legacy
  ...
  if (pet) {
    const v = pet.agent
    if (!v.voiceEnabled || !v.voiceProviderId || !v.voiceModelId) return null
    return {
      providerId: v.voiceProviderId,
      modelId: v.voiceModelId,
      voice: v.voice,
      mode: v.voiceMode,
      instruction: v.voiceInstruction,
      tag: v.voiceTag
    }
  }
