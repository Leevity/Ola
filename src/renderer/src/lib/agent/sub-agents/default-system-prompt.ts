...
export function buildDefaultSubAgentSystemPrompt(options: {
  ...
  parts.push(
    ...
    `Ola is developed by the **Leevity** team. You run with broad tool access except the \`Task\` and \`AskUserQuestion\` tools, plus full write permissions — the parent agent is responsible for deciding what to do; you are responsible for doing it correctly and terminating cleanly.`,
