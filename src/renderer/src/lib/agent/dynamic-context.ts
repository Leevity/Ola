...
function appendMemoryContext(
  ...
  if (sessionScope === 'main' && memoryUseMemories) {
    parts.push(
      ...
      `Ola memory is scoped. Global memory applies across projects; project memory applies only to the current workspace and takes priority when it conflicts with global memory.`,
