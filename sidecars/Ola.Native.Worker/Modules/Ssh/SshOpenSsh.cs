...
internal static class SshOpenSsh
{
    ...
    private const string ControlSocketDirUnix = "/tmp/ola-ssh";
        ...
        int maxStderrChars = DefaultMaxStderrChars)
    {
        ...
        var tempPath = Path.Combine(
            ...
            $".{Path.GetFileName(targetPath)}.ola-{Guid.NewGuid():N}.tmp");
    ...
    private static string? ConfigureAskPass(ProcessStartInfo startInfo, string secret)
    {
        ...
        else
        {
            helperPath = Path.Combine(
                Path.GetTempPath(),
                $"ola-ssh-askpass-{Guid.NewGuid():N}.sh");
            File.WriteAllText(
                helperPath,
                "#!/bin/sh\nprintf '%s\\n' \"${OLA_SSH_ASKPASS_SECRET}\"\n");
