...
internal static class DbSchemaMigrator
{
    ...
    private static void ApplyAdditiveMigrations(SqliteConnection connection)
    {
        ...
        // after ALTER TABLE ADD COLUMN. Split the post-migration UPDATEs into fresh commands
        // so each one re-prepares against the current schema.
        Execute(
            connection,
            "UPDATE sessions SET model_selection_mode = 'manual' WHERE provider_id IS NOT NULL AND model_id IS NOT NULL AND model_selection_mode = 'inherit'");
        Execute(
            connection,
        ...
        string sql)
    {
        ...
        {
            WorkerLog.Warn($"db migration SQL failed: {ex.Message} | SQL: {sql}");
            throw;
        }
    }
