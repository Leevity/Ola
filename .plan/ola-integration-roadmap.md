...
> 状态：阶段 0–21 已完成本地实现、分阶段提交与自动门禁；阶段 11–21 已按独立分支实施。阶段 22 定义下一版本的产品收敛与可信交付门槛。最终远端上传门禁执行中。
...
## 阶段 22：可信 Agent 工作台收敛

### 产品范围

- 北极星用户为独立开发者与小型技术团队。
- 产品主叙事为：让 Agent 在本地和远程工作区完成可审阅的工作，并按需接入团队工具。
- 在本阶段验收完成前，不新增与核心工作流无关的一级产品能力；Draw、媒体、桌宠保持可选工作台定位，不扩展核心导航和默认引导。

### 三条必须闭环的主路径

1. 本地代码：理解问题 → 制定计划 → 修改 → 验证 → 汇报。
2. 远程诊断：连接 → 诊断 → 受控执行 → 留痕 → 回传。
3. 自动化通知：Cron 或渠道触发 → 审批 → 执行 → 结果送达。

### 交付门槛

- 所有本地打包命令必须先发布 Native Worker 与 CodeGraph Worker，并严格校验运行资产。
- 每条主路径必须提供首次成功引导、连通性检查、失败修复说明和明确完成状态。
- 高影响操作必须能关联工作区或远程主机、触发来源、审批状态、命令或变更摘要、产物及失败原因。
- 打包验收至少覆盖：Worker 资产存在、未打包应用可启动、数据库与 Worker 握手成功。

### 后续 P1 交付切片

#### 阶段 23：统一执行审计

- 基于现有聊天执行摘要、`agent_change_sets`、Cron run/logs 和审批状态建立单一 Execution Record 读取模型，不复制第二套运行历史。
- 首版覆盖本地聊天与 Cron：来源、工作区或 SSH 主机、审批状态、工具/命令摘要、文件变更、产物、失败原因和跳转入口。
- 后续扩展到 Channels、Extensions、MCP；敏感字段只能保存脱敏摘要。
- 验收：一次文件修改、一次失败命令、一次 Cron 投递都可以从统一记录定位上下文、结果和失败原因。

#### 阶段 24：三条场景化首次成功路径

- 本地代码路径：项目选择、Provider 连通性、权限说明、首个“计划 → 修改 → 验证 → 汇报”任务。
- 远程诊断路径：SSH 连通性与最小权限检查、受控命令确认、审计回传。
- 自动化通知路径：Cron、一个渠道、投递测试和失败修复指引。
- 验收：用户不必理解全部 Settings，也能在 15 分钟内完成任一路径并看到明确完成状态。

#### 阶段 25：能力生命周期与发布支持边界

- 为能力声明 Prototype、Internal、Beta、GA、Deprecated 状态，并在设置、文档和发布说明中保持一致。
- GA 门槛至少包括：权限审查、可观测性、错误恢复、支持矩阵、端到端验证和安装包冒烟。
- Cookie 导入、远程运维、渠道机器人、视频生成等高风险能力默认按 Beta 管理，直到满足 GA 门槛。
