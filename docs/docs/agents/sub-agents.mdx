---
title: 子代理
description: Ola 的 Task 子代理系统、内置 agent 和自定义 agent。
---

# 子代理 / Sub-Agents

子代理是 Ola 用来处理“复杂但可独立拆分”的任务的方式。核心入口是 `Task` 工具，它会根据 `subagent_type` 启动一个专门的 agent。

## 子代理从哪里来？ / Where do they come from?

Ola 会在首次运行时把内置 agent 从 `resources/agents/` 同步到：
