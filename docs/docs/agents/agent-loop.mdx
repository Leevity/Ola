---
title: Agent 循环
description: Ola 的核心执行循环、流式事件和工具调用机制。
---

# Agent 循环 / Agent Loop

Agent Loop 是 Ola 的核心执行引擎，入口位于 `src/renderer/src/lib/agent/agent-loop.ts`。它是一个 `AsyncGenerator`：每轮都会把消息发给 provider、解析流式事件、执行工具、再把工具结果追加回消息队列。
