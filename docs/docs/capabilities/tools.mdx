---
title: 工具系统
description: Ola 工具注册、调用、审批和完整工具分类。
---

# 工具系统 / Tool System

工具系统是 Ola 把“聊天”变成“执行”的关键。入口位于 `src/renderer/src/lib/tools/`，统一通过 `toolRegistry` 注册。
...
4. 动态目录刷新（skills / sub-agents / extensions）
5. Team 工具
6. Web Search 动态注册
7. 应用插件工具动态注册
8. MCP 工具运行时发现
...
Extensions 是用户安装的本地插件。启用后，工具会以 `extension__<extensionId>__<toolName>` 命名动态注册。

- 声明式 HTTP 工具走 `extension:fetch`
- JS handler 在 sandbox iframe 中运行
- 结果可返回 `ui.kind = card / table / form / chart / html`
...
- `Task` 从 `~/.ola/agents/` 加载子代理定义
- `Extension` 从 `~/.ola/extensions/` 扫描 manifest
...
Ola 的工具系统本质上是一个**可注册、可动态扩展、带审批策略的能力总线**。Agent 不是“猜怎么做”，而是“拿工具去做”。
