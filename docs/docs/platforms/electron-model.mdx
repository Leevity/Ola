---
title: Electron 进程模型
description: Ola 的主进程、渲染进程、预加载脚本和共享契约如何分工。
---

# Electron 进程模型 / Electron Process Model

Ola 的 Electron 架构不是“一个窗口 + 一个渲染页”这么简单，而是一个有明确边界的四层系统：**Main、Preload、Renderer、Shared**。
...
这四层分工，是 Ola 能同时兼顾“可用性”和“可控性”的基础。
