---
title: 数据库设计
description: Ola SQLite 数据库的表结构、业务域和迁移策略。
---

# 数据库设计 / Database Design

Ola 使用 **SQLite + Native Worker** 作为本地持久化核心。数据库文件位于
`~/.ola/data.db`。`src/main/db/database.ts` 只负责发起 `db/initialize`，实际连接、查询和迁移
由 `sidecars/Ola.Native.Worker/Modules/Db/` 承担。

## 基础配置 / Database config

| 项       | 值                    |
| -------- | --------------------- |
| 引擎     | Native Worker SQLite  |
| 位置     | `~/.ola/data.db`      |
| 模式     | WAL                   |
| 外键     | `foreign_keys = ON`   |
| 迁移方式 | 集中建表 + 幂等列补齐 |
...
- 文档里如果看到旧 schema，应该以
  `sidecars/Ola.Native.Worker/Modules/Db/DbSchemaMigrator.cs` 为准
